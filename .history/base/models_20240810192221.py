...
class Inventory(models.Model):
    ...
    def increment_sold_count(self):
        ...
        profile.sold_count += 1  # Increment by 1 per item sold, not by quantity
        profile.save()

    def increment_inventory_count(self):
        profile = self.get_profile()
        if self.status == 'Available':
            profile.inventory_count += self.quantity  # Add items to inventory
        elif self.status == 'Sold':
            profile.inventory_count -= self.quantity  # Subtract items from inventory when sold
        profile.save()
