    ...
    def increment_inventory_count(self, is_inventory_page=False, is_edit_page=False):
        ...
        if is_edit_page:
            if self.quantity > original_quantity:
                # If the new quantity is greater than the original, add the difference to the inventory count
                ...
                # If the new quantity is less than the original, subtract the difference from the inventory count
        ...
        elif is_edit_page and self.status == 'Available':
            # Logic for handling edits where quantity changes
            original_quantity = Inventory.objects.get(pk=self.pk).quantity
            if self.quantity != original_quantity:
                diff = self.quantity - original_quantity
                profile.inventory_count += diff
                print(f"Debug: Updated inventory by {diff} items. New inventory count: {profile.inventory_count}")
