...
class Inventory(models.Model):
    ...
    def increment_inventory_count(self, is_inventory_page=False, is_edit_page=False):
        ...
        if self.pk:  # This means the item already exists
            ...
            original_status = None  # No original status for a new item
        ...
        if is_inventory_page:
            ...
            if original_status == "Available":
                ...
                # If the original status was 'Sold' and now it's 'Available', add the quantity back to inventory
                profile.inventory_count += self.quantity
                print(f"Debug: Changed status from Sold to Available. Added {self.quantity} items back to inventory.")
