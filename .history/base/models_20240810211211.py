...
class Inventory(models.Model):
    ...
    def save(self, *args, **kwargs):
        ...
        creating = not bool(self.pk)  # Determine if this is a new item
        if not creating:
        ...
        # Increment the profile's sold count and profit count only once
        if creating or (not creating and self.status == 'Sold'):
            self.increment_profit_count()
            self.increment_inventory_count(is_inventory_page=is_inventory_page, is_edit_page=is_edit_page, original_quantity=original_quantity)
    ...
    def increment_inventory_count(self, is_inventory_page=False, is_edit_page=False, original_quantity=0):
        profile = self.get_profile()
