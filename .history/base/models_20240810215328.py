...
class Inventory(models.Model):
    ...
    sold_quantity = models.PositiveIntegerField(default=0)  # New field to track sold items
    ...
    def save(self, *args, **kwargs):
        ...
        if self.pk:
            ...
            original_sold_quantity = Inventory.objects.get(pk=self.pk).sold_quantity
            ...
            if original_status != 'Sold' and self.status == 'Sold':
                ...
                print(f"Debug: Status changed to Sold. Incrementing sold count.")
                self.increment_sold_count()
        elif self.status == 'Sold':
            self.sold_quantity += self.quantity
            print(f"Debug: New item with status Sold. Incrementing sold count.")
            self.increment_sold_count()
    ...
    def increment_inventory_count(self):
        ...
        available_quantity = self.quantity - self.sold_quantity  # Calculate available quantity

        if self.status == 'Available':
            profile.inventory_count += available_quantity
            print(f"Debug: Added {available_quantity} items to inventory. New inventory count: {profile.inventory_count}")
        
        elif self.status == 'Sold':
            if profile.inventory_count >= self.sold_quantity:
                profile.inventory_count -= self.sold_quantity  # Subtract sold items from inventory
