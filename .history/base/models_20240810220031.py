...
class Inventory(models.Model):
    ...
    def save(self, *args, **kwargs):
        if self.pk:
            ...
            if original_status != 'Sold' and self.status == 'Sold':
                ...
                self.quantity -= self.sold_quantity  # Reduce available quantity by sold quantity
            elif original_status == 'Sold' and self.status == 'Available':
                self.quantity += original_sold_quantity  # Restore the available quantity when reverting to available
                self.sold_quantity = 0  # Reset sold quantity
        else:
            if self.status == 'Sold':
                self.sold_quantity += self.quantity
                self.quantity = 0  # Reduce available quantity to 0 when first marking as sold
    ...
    def increment_inventory_count(self):
        ...
        if self.status == 'Available':
            ...
            profile.inventory_count -= self.sold_quantity
