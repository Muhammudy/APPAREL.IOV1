...
class Inventory(models.Model):
    ...
    sold_quantity = models.PositiveIntegerField(default=0)  # Track how many items are sold
    ...
    def save(self, *args, **kwargs):
        if self.pk:
            ...
            if original_status != 'Sold' and self.status == 'Sold':
                ...
                self.quantity = 0  # Set quantity to 0 as all items are sold
        else:
            if self.status == 'Sold':
...
class Profile(models.Model):
    ...
    instagram = models.URLField(maximum_length=200, blank=True)
    inventory_link = models.URLField(maximum_length=200, blank=True)
