...
class Inventory(models.Model):
    ...
    sold_quantity = models.PositiveIntegerField(default=0, editable=False)  # New field to track sold items
    ...
    def save(self, *args, **kwargs):
        if self.pk:
            ...
            if original_status != 'Sold' and self.status == 'Sold':
                # Change to Sold, update sold_quantity and reduce available quantity
                self.sold_quantity += self.quantity
                self.quantity = 0  # Reduce available quantity to zero
                ...
                # Reverting to Available, restore sold_quantity to available quantity
                self.quantity += self.sold_quantity
                self.sold_quantity = 0  # Reset sold_quantity
                print("Debug: Status reverted to Available. Restoring quantity.")
        ...
        if self.price_sold and self.price_sold > Decimal(0):
            total_paid = Decimal(self.price_paid) * (self.quantity + self.sold_quantity)
