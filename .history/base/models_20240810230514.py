...
class Inventory(models.Model):
    ...
    def increment_inventory_count(self):
        ...
        if self.status == 'Available':
            # When the item is available, the quantity should be added to the inventory count
            ...
            if original_status == 'Available' and original_quantity > 0:
                # If the item was originally available, and now it's being marked as sold in the edit page
                profile.inventory_count -= original_quantity  # Subtract the original quantity from inventory
                print(f"Debug: Subtracted {original_quantity} items from inventory. New inventory count: {profile.inventory_count}")
            else:
                # If the item is newly created as sold, do not affect inventory count
                print(f"Debug: Item marked as Sold without affecting inventory count since it's newly created or already sold.")
