...
class Inventory(models.Model):
    ...
    def delete(self, *args, **kwargs):
        ...
        if self.status == 'Available':
            ...
            # Update metrics after adjusting the inventory count
            self.update_metrics_after_delete(profile)

        elif self.status == 'Sold':
            # Adjust sold count and profit before deletion
            profile.sold_count -= self.sold_quantity
            profile.profit_count -= self.profit
            profile.save()

            # Update metrics after adjusting the sold count and profit
            self.update_metrics_after_delete(profile)
