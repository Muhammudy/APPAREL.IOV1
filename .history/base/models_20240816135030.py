...
class Inventory(models.Model):
    ...
    def delete(self, *args, **kwargs):
        ...
        if self.status == 'Available':
            # Adjust inventory count before deletion
            profile.inventory_count -= self.quantity
            profile.save()


        # Update metrics after adjusting the counts
        self.update_metrics_after_delete(profile)

        super(Inventory, self).delete(*args, **kwargs)

    def update_metrics_after_delete(self, profile):
...
class Profile(models.Model):
    ...
    company_logo = models.ImageField(default='company_pic/logo.png')
...
class Home(models.Model):
    ...
    def __str__(self):
        return str(self.company_logo)  # Ensuring this returns a string
