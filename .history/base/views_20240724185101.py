...
def inventory(request):
    if request.method == 'POST':
        ...
        if form.is_valid():
            logger.debug('Form is valid. Saving form data.')
            form.save()
    ...
    logger.debug('Rendering inventory.html template with context.')
...
def updateInventory(request, pk):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            form.save()
            return redirect('inventory')  # Redirect to the inventory list view
    else:
