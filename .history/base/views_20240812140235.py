...
def calculate_sales_percentage(metrics):
    if not metrics or metrics.sold_count == 0:
        return 0
    # Example calculation - customize based on your logic
    return (metrics.sold_count / some_total_sales) * 100

def calculate_inventory_percentage(metrics):
    if not metrics or metrics.inventory_count == 0:
        return 0
    return (metrics.inventory_count / some_total_inventory) * 100

def calculate_profit_percentage(metrics):
    if not metrics or metrics.profit_count == 0:
        return 0
    return (metrics.profit_count / some_total_profit) * 100
...
def dashboard(request):
    ...
    context = {
        ...
        'sales_percentage': calculate_sales_percentage(daily_metrics),
        'inventory_percentage': calculate_inventory_percentage(daily_metrics),
        'profit_percentage': calculate_profit_percentage(daily_metrics),
    }
