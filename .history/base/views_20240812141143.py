...
def dashboard(request):
    ...
    daily_metrics = DailyMetrics.objects.filter(user=request.user, date=today).first()
    monthly_metrics = MonthlyMetrics.objects.filter(user=request.user).order_by('-year', '-month').first()
    yearly_metrics = YearlyMetrics.objects.filter(user=request.user).order_by('-year').first()
    ...
    total_sales = daily_metrics.sold_count if daily_metrics else 0
    total_inventory = daily_metrics.inventory_count if daily_metrics else 0
    total_profit = daily_metrics.profit_count if daily_metrics else 0

    daily_sales_percentage = calculate_sales_percentage(daily_metrics, total_sales)
    daily_inventory_percentage = calculate_inventory_percentage(daily_metrics, total_inventory)
    daily_profit_percentage = calculate_profit_percentage(daily_metrics, total_profit)

    monthly_sales_percentage = calculate_sales_percentage(monthly_metrics, total_sales)
    monthly_inventory_percentage = calculate_inventory_percentage(monthly_metrics, total_inventory)
    monthly_profit_percentage = calculate_profit_percentage(monthly_metrics, total_profit)

    yearly_sales_percentage = calculate_sales_percentage(yearly_metrics, total_sales)
    yearly_inventory_percentage = calculate_inventory_percentage(yearly_metrics, total_inventory)
    yearly_profit_percentage = calculate_profit_percentage(yearly_metrics, total_profit)
